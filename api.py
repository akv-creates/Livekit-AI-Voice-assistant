...
class AssistantFnc(llm.FunctionContext):
    ...
    @llm.ai_callable(description="generate a unique id for a purpose")
    def generate_unique_id(
        self,
        purpose: Annotated[str, llm.TypeInfo(description="Purpose for which to generate the id")] = "session",
    ):
        id_str = str(uuid.uuid4())
        self._last_generated_id = id_str
        logger.info("generated id for %s: %s", purpose, id_str)
        return f"Generated unique id for {purpose}: {id_str}"